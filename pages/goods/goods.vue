<template>
	<view>
		...
		<!-- 模态框 -->
		<!-- <popupSpec @add="add" @sub="sub" :spaceInfo="spaceInfo" :goodsInfo="goodsInfo" :goodsData="goodsData" @setSelectSpec="setSelectSpec" @hideSpec="hideSpec" /> -->
		...
		<view class="info-box comments">
			...
			<view v-if="goodsData.comment.length != 0" class="comment" @tap="handleRatings">
				<view class="user-info">
					<view class="face">
						<image :src="goodsData.comment[0].avatar"></image>
					</view>
					<view class="username">{{goodsData.comment[0].name}}</view>
				</view>
				<view class="content">
					{{goodsData.comment[0].c_comment}}
				</view>
			</view>
			<view v-else class="comment">暂无卖家评价</view>
		</view>
		<!-- 商品详情 -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.goodsID = JSON.parse(option.goodsID);
			//console.log("get data:",this.goodsID);
		...
		methods: {
			...
			initData(params) {
				this.request({
					...
					success: ((res) => {
						console.log("===>res",res)
						this.goodsData.swiperList = res.swiperList
						this.goodsInfo.name = res.bookDetail[0].name
						this.goodsInfo.price = res.bookDetail[0].price
						this.goodsInfo.img = res.swiperList[0].url
						this.goodsInfo.id = res.bookDetail[0].id
						this.goodsData.descriptionStr = res.bookDetail[0].description
						console.log("getgoodsData",this.goodsData)
						this.request({
							url:interfaces.getcomment,
							method:"GET",
							header:{
								'Content-Type': 'application/json'
							},
							...
							success:((res)=>{
								this.goodsData.comment = res.res
			...
			// counter--popupSpec--goods(子--父--父)(goodsInfo是在goods定义  需要在goods里改 否则app端点击无反应)
			add() {
			...
			joinCart() {
				...
				uni.getStorage({
					...
					success: (res => {
						...
						// 查找商品是否存在
						let isExist = false;
						console.log("Storage goodList:", goodsList)
						console.log("this.goodInfo:", this.goodsInfo)
						goodsList.forEach(goods => { // 如果添加的商品id和规则在购物车已经存在
							console.log(goods.id)
							if (goods.id == this.goodsInfo
								.goods_id /*&& goods.spec == this.goodsInfo.spec*/ ) {
								// 如果存在  修改商品数量
