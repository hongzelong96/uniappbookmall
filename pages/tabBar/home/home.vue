	...
	<view>
		...
		<view class="swiper">
			<view class="swiper-box">
				<swiper class="swiper" circular="true" autoplay="true" @change="swiperChange" :interval="interval"
					:duration="duration">
				...
				<view class="indicator">
					<view class="dots" :class="{'on': currentSwiper >= index}" v-for="(swiper, index) in swiperList"
						:key="index"></view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			handleCategory(item) {
				...
				uni.navigateTo({
					url: "../../goods/goodsList?name=" + item.name + "&categoryId=" + item.id
				})
			},
			// 活动区跳转
			handlePromotion(item) {
				uni.showToast({
					title: "跳转到 " + item.title + " 页面",
					icon: 'none'
				})
			},
		}
